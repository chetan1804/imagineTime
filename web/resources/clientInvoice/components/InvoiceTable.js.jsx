...
import MyDocument from "./PdfDocument.js.jsx";
  ...
  render() {
    ...
    const handleInvoiceModal = (type, data) => {
      ...
      if (type === "edit") {
        ...
        defaultObj["payment_due_date"] = moment().utc().utc().format();
        defaultObj["invoice_date"] = moment().utc().utc().format();
        ...
        defaultObj["firm_id"] = parseInt(match.params.firmId);
    ...
    const handleSendEmail = () => {
      const clientEmail = userStore.byId[selectedClient._primaryContact];
      const format = "MM-DD-YYYY";
      const emailData = {
        email: clientEmail.username,
        firmId: match.params.firmId,
        firmName: selectedFirm && selectedFirm.name,
        invoiceDate: moment(selectedData && selectedData.invoice_date).utc().utc().format(
          format
        ),
        invoiceDueDate: moment(
          selectedData && selectedData.payment_due_date
        ).utc().utc().format(format),
        invoiceNum: selectedData && selectedData.invoice_number,
        invoiceAmt: selectedData && selectedData.invoice_amount,
        invoiceBal: selectedData && selectedData.invoice_balance,
        clientName: selectedData && selectedData.name,
        payNowToken: selectedData && selectedData.paynow_token,
      };
      handleAlertModal();
      setTimeout(() => {
        ...
        dispatch(paymentActions.sendEmail(emailData));
      }, 2000);
    };
    ...
    const handleDelete = () => {
      dispatch(
        deleteInvoice(
          selectedData && selectedData.invoice_id,
