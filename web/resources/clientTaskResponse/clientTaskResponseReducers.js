/**
 * Build the ClientTaskResponse store
 *
...
// import clientTaskResponse actions
import * as Actions from './clientTaskResponseActions';
import apiUtils from '../../global/utils/api'

/**
 * clientTaskResponseList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'clientTaskResponse' reducer (defined
 * below) when dealing with a LIST action
 */
function clientTaskResponseList(state = {
  /**
   ...
   * then recursively return an _additional_ clientTaskResponseList reducer.
   *
   * Otherwise, return the actual clientTaskResponse lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientTaskResponseList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: clientTaskResponseList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_TASK_RESPONSE_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_CLIENT_TASK_RESPONSE_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_CLIENT_TASK_RESPONSE_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_CLIENT_TASK_RESPONSE_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_TASK_RESPONSE_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_TASK_RESPONSE_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_TASK_RESPONSE_PAGINATION: {
        return {
          ...state
...
/**
 * Primary clientTaskResponse reducer -
 *
 * This is the single source of truth for all things 'clientTaskResponse' related within the
 ...
 */
function clientTaskResponse(state = {

  /**
   * "byId" is an object map of all clientTaskResponse items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ clientTaskResponse
   * any component that creates a new clientTaskResponse object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the clientTaskResponseList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a clientTaskResponse, the single clientTaskResponse
   ...
   *
   * For example, when fetching a nested list of clientTaskResponses by type and color you
   * would write something like:
   * let list = clientTaskResponseStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT CLIENT_TASK_RESPONSE ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_TASK_RESPONSE: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_TASK_RESPONSE: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_TASK_RESPONSE_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_TASK_RESPONSE_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_TASK_RESPONSE ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_TASK_RESPONSE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_TASK_RESPONSE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_TASK_RESPONSE_TO_MAP: {
      // add this clientTaskResponse to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_CLIENT_TASK_RESPONSE: {
      // add this clientTaskResponse to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT_TASK_RESPONSE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT_TASK_RESPONSE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_CLIENT_TASK_RESPONSE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_CLIENT_TASK_RESPONSE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_TASK_RESPONSE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT_TASK_RESPONSE: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_TASK_RESPONSE: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_CLIENT_TASK_RESPONSE_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: clientTaskResponseList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
...
export default clientTaskResponse;
