 ...
 *
 * NOTE: this is never called directly. Only by parent 'notification' reducer (defined
 * below) when dealing with a LIST action
 */
function notificationList(state = {
  /**
   ...
   * then recursively return an _additional_ notificationList reducer.
   *
   * Otherwise, return the actual notification lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional notificationList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: notificationList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_NOTIFICATION_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_NOTIFICATION_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_NOTIFICATION_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_NOTIFICATION_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_NOTIFICATION_FROM_LIST: {
      ...
      case Actions.SET_NOTIFICATION_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_NOTIFICATION_PAGINATION: {
        return {
          ...state
...
/**
 * Primary notification reducer -
 *
 * This is the single source of truth for all things 'notification' related within the
...
function notification(state = {
  ...
  /**
   * "defaultItem" defines fields for a _new_ notification
   * any component that creates a new notification object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the notificationList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a notification, the single notification
   ...
   *
   * For example, when fetching a nested list of notifications by type and color you
   * would write something like:
   * let list = notificationStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_NOTIFICATION: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_NOTIFICATION: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_NOTIFICATION_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_NOTIFICATION_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_NOTIFICATION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_NOTIFICATION: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_NOTIFICATION_TO_MAP: {
      // add this notification to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_NOTIFICATION: {
      // add this notification to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_NOTIFICATION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_NOTIFICATION: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_NOTIFICATION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_NOTIFICATION: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_NOTIFICATION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_NOTIFICATION: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_NOTIFICATION: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_NOTIFICATION_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: notificationList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
