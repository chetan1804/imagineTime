...
import {CLIENT_ACCOUNT_TYPES, CLIENT_ENGAGEMENT_TYPES} from '../../config/constants';
 ...
 *
 * NOTE: this is never called directly. Only by parent 'client' reducer (defined
...
function clientList(state = {
   ...
   * then recursively return an _additional_ clientList reducer.
   *
   * Otherwise, return the actual client lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: clientList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
    ...
    switch(action.type) {
      ...
      case FirmActions.RECEIVE_FIRM_LIST: {
        if(!action.clients) {
          ...
          let idArray = [];
          for(const item of action.clients) {
            idArray.push(item._id);
          }
          return {
            ...state
      ...
      case Actions.ADD_CLIENT_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_QUERY: {
        return {
          ...state
          , query: action.query
        }
      }
      case Actions.SET_CLIENT_PAGINATION: {
        return {
          ...state
...
/**
 * Primary client reducer -
 *
 * This is the single source of truth for all things 'client' related within the
...
function client(state = {
  ...
  /**
   * "byId" is an object map of all client items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ client
   * any component that creates a new client object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  , formHelpers: {
    ...
    , engagementTypes: CLIENT_ENGAGEMENT_TYPES
  }

  /**
   * "lists" corresponds to individual instances of the clientList reducer as
   * defined above.
   *
  ...
  /**
   * Names, and their default values, of the filters that can be applied on
   * the Client Settings screen.
   */
  , filterNames: {engagementTypeFilter: null}
  /**
   * "selected" is a single _selected_ entity within the store
   *
   * For example, when changing the name of a client, the single client
   ...
   *
   * For example, when fetching a nested list of clients by type and color you
   * would write something like:
   * let list = clientStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_CLIENT: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_CLIENT: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_TO_MAP: {
    ...
    case Actions.SET_SELECTED_CLIENT: {
      // add this client to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.RECEIVE_UPDATE_CLIENT: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_BULK_UPDATE_CLIENT: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_BULK_UPDATE_CLIENT: {
    ...
    case Actions.REQUEST_DELETE_CLIENT: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
     ...
     */
    case Actions.INVALIDATE_CLIENT_LIST: {
      if (action && action.listArgs && action.listArgs.includes("all")) {
        nextState = {
          ...state
          , lists: {}
        }
        ...
        nextState = {
          ...
          , lists: {
            ...state.lists
            , [action.listArgs[0]]: clientList(state.lists[action.listArgs[0]] || {}, action)
          }
        }
      }
      break;
    ...
    case Actions.RECEIVE_CLIENT_LIST: {
      ...
      if (action.engagementTypes && action.engagementTypes.length) {
        nextState.formHelpers.engagementTypes = action.engagementTypes;
      }
      break;
    }
    case FirmActions.RECEIVE_FIRM_LIST: {
      console.log("client list received from external source")
      // add items to "byId" before we forward to individual list reducer
      let newIdMap = { ...state.byId };
      if(action.success && action.clients) {
        for(const item of action.clients) {
          newIdMap[item._id] = item;
        }
      }
    ...
    case Actions.SET_CLIENTSETTINGS_FILTER: {
      nextState = {
        ...state
