...
// import documentTemplate actions
import * as Actions from './documentTemplateActions';
import apiUtils from '../../global/utils/api'

/**
 * documentTemplateList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'documentTemplate' reducer (defined
 * below) when dealing with a LIST action
 */
function documentTemplateList(state = {
  /**
   ...
   * then recursively return an _additional_ documentTemplateList reducer.
   *
   * Otherwise, return the actual documentTemplate lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional documentTemplateList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: documentTemplateList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_DOCUMENT_TEMPLATE_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_DOCUMENT_TEMPLATE_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_DOCUMENT_TEMPLATE_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_DOCUMENT_TEMPLATE_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_DOCUMENT_TEMPLATE_FROM_LIST: {
      ...
      case Actions.SET_DOCUMENT_TEMPLATE_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_DOCUMENT_TEMPLATE_PAGINATION: {
        return {
          ...state
          , pagination: action.pagination
        }
      }
      case Actions.SET_DOCUMENT_TEMPLATE_QUERY: {
        return {
          ...state
          , query: action.query
        }
...
/**
 * Primary documentTemplate reducer -
 *
 * This is the single source of truth for all things 'documentTemplate' related within the
 ...
 */
function documentTemplate(state = {

  /**
   * "byId" is an object map of all documentTemplate items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ documentTemplate
   * any component that creates a new documentTemplate object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the documentTemplateList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a documentTemplate, the single documentTemplate
   ...
   *
   * For example, when fetching a nested list of documentTemplates by type and color you
   * would write something like:
   * let list = documentTemplatestore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT DOCUMENT_TEMPLATE ACTIONS
     */
    case Actions.REQUEST_DEFAULT_DOCUMENT_TEMPLATE: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_DOCUMENT_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_DOCUMENT_TEMPLATE_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DOCUMENT_TEMPLATE_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE DOCUMENT_TEMPLATE ACTIONS
     */
    case Actions.REQUEST_SINGLE_DOCUMENT_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_DOCUMENT_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_DOCUMENT_TEMPLATE_TO_MAP: {
      // add this documentTemplate to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_DOCUMENT_TEMPLATE: {
      // add this documentTemplate to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_DOCUMENT_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_DOCUMENT_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_DOCUMENT_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_DOCUMENT_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_DOCUMENT_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_DOCUMENT_TEMPLATE: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_DOCUMENT_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_DOCUMENT_TEMPLATE_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: documentTemplateList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
