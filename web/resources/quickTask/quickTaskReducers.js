...
// import quickTask actions
import * as Actions from './quickTaskActions';
import apiUtils from '../../global/utils/api'

/**
 * quickTaskList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'quickTask' reducer (defined
 * below) when dealing with a LIST action
 */
function quickTaskList(state = {
  /**
   ...
   * then recursively return an _additional_ quickTaskList reducer.
   *
   * Otherwise, return the actual quickTask lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional quickTaskList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: quickTaskList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_QUICK_TASK_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_QUICK_TASK_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_QUICK_TASK_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_QUICK_TASK_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_QUICK_TASK_FROM_LIST: {
      ...
      case Actions.SET_QUICK_TASK_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_QUICK_TASK_PAGINATION: {
        return {
          ...state
...
/**
 * Primary quickTask reducer -
 *
 * This is the single source of truth for all things 'quickTask' related within the
 ...
 */
function quickTask(state = {

  /**
   * "byId" is an object map of all quickTask items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ quickTask
   * any component that creates a new quickTask object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  , formHelpers: {
    types: ['file', 'signature']
  }

  /**
   * "lists" corresponds to individual instances of the quickTaskList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a quickTask, the single quickTask
   ...
   *
   * For example, when fetching a nested list of quickTasks by type and color you
   * would write something like:
   * let list = quickTaskStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT QUICK_TASK ACTIONS
     */
    case Actions.REQUEST_DEFAULT_QUICK_TASK: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_QUICK_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_QUICK_TASK_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_QUICK_TASK_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE QUICK_TASK ACTIONS
     */
    case Actions.REQUEST_SINGLE_QUICK_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_QUICK_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_QUICK_TASK_TO_MAP: {
      // add this quickTask to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_QUICK_TASK: {
      // add this quickTask to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_QUICK_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_QUICK_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_QUICK_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_QUICK_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_QUICK_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_QUICK_TASK: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_QUICK_TASK: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_QUICK_TASK_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: quickTaskList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
