/**
 * Build the ClientActivity store
 *
...
// import clientActivity actions
import * as Actions from './clientActivityActions';
import apiUtils from '../../global/utils/api'

/**
 * clientActivityList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'clientActivity' reducer (defined
 * below) when dealing with a LIST action
 */
function clientActivityList(state = {
  /**
   ...
   * then recursively return an _additional_ clientActivityList reducer.
   *
   * Otherwise, return the actual clientActivity lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientActivityList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: clientActivityList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_ACTIVITY_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_CLIENT_ACTIVITY_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_CLIENT_ACTIVITY_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_CLIENT_ACTIVITY_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_ACTIVITY_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_ACTIVITY_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_ACTIVITY_PAGINATION: {
        return {
          ...state
...
/**
 * Primary clientActivity reducer -
 *
 * This is the single source of truth for all things 'clientActivity' related within the
 ...
 */
function clientActivity(state = {

  /**
   * "byId" is an object map of all clientActivity items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ clientActivity
   * any component that creates a new clientActivity object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the clientActivityList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a clientActivity, the single clientActivity
   ...
   *
   * For example, when fetching a nested list of clientActivitys by type and color you
   * would write something like:
   * let list = clientActivityStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT CLIENT_ACTIVITY ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_ACTIVITY: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_ACTIVITY_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_ACTIVITY_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_ACTIVITY ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_ACTIVITY_TO_MAP: {
      // add this clientActivity to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_CLIENT_ACTIVITY: {
      // add this clientActivity to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_CLIENT_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_CLIENT_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT_ACTIVITY: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_CLIENT_ACTIVITY_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: clientActivityList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
