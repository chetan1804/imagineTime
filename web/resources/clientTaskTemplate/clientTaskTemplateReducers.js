/**
 * Build the ClientTaskTemplate store
 *
...
// import clientTaskTemplate actions
import * as Actions from './clientTaskTemplateActions';
import apiUtils from '../../global/utils/api'

/**
 * clientTaskTemplateList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'clientTaskTemplate' reducer (defined
 * below) when dealing with a LIST action
 */
function clientTaskTemplateList(state = {
  /**
   ...
   * then recursively return an _additional_ clientTaskTemplateList reducer.
   *
   * Otherwise, return the actual clientTaskTemplate lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientTaskTemplateList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: clientTaskTemplateList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_TASK_TEMPLATE_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_CLIENT_TASK_TEMPLATE_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_CLIENT_TASK_TEMPLATE_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_CLIENT_TASK_TEMPLATE_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_TASK_TEMPLATE_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_TASK_TEMPLATE_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_TASK_TEMPLATE_PAGINATION: {
        return {
          ...state
...
/**
 * Primary clientTaskTemplate reducer -
 *
 * This is the single source of truth for all things 'clientTaskTemplate' related within the
 ...
 */
function clientTaskTemplate(state = {

  /**
   * "byId" is an object map of all clientTaskTemplate items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ clientTaskTemplate
   * any component that creates a new clientTaskTemplate object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the clientTaskTemplateList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a clientTaskTemplate, the single clientTaskTemplate
   ...
   *
   * For example, when fetching a nested list of clientTaskTemplates by type and color you
   * would write something like:
   * let list = clientTaskTemplateStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT CLIENT_TASK_TEMPLATE ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_TASK_TEMPLATE: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_TASK_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_TASK_TEMPLATE_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_TASK_TEMPLATE_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_TASK_TEMPLATE ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_TASK_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_TASK_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_TASK_TEMPLATE_TO_MAP: {
      // add this clientTaskTemplate to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_CLIENT_TASK_TEMPLATE: {
      // add this clientTaskTemplate to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT_TASK_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT_TASK_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_CLIENT_TASK_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_CLIENT_TASK_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_TASK_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT_TASK_TEMPLATE: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_TASK_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_CLIENT_TASK_TEMPLATE_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: clientTaskTemplateList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
