/**
 * Build the FolderTemplate store
 *
...
// import folderTemplate actions
import * as Actions from './folderTemplateActions';
...
/**
 * folderTemplateList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'folderTemplate' reducer (defined
 * below) when dealing with a LIST action
 */
function folderTemplateList(state = {
  /**
   ...
   * then recursively return an _additional_ folderTemplateList reducer.
   *
   * Otherwise, return the actual folderTemplate lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional folderTemplateList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: folderTemplateList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_FOLDER_TEMPLATE_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_FOLDER_TEMPLATE_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_FOLDER_TEMPLATE_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case UserActions.RECEIVE_USER_LIST: {
        if(!action.folderTemplates) {
          ...
          let idArray = [];
          for(const item of action.folderTemplates) {
            idArray.push(item._id);
          }
          return {
            ...state
      ...
      case Actions.ADD_FOLDER_TEMPLATE_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_FOLDER_TEMPLATE_FROM_LIST: {
      ...
      case Actions.SET_FOLDER_TEMPLATE_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_FOLDER_TEMPLATE_QUERY: {
        return {
          ...state
          , query: action.query
        }
      }
      case Actions.SET_FOLDER_TEMPLATE_PAGINATION: {
        return {
          ...state
...
/**
 * Primary folderTemplate reducer -
 *
 * This is the single source of truth for all things 'folderTemplate' related within the
 ...
 */
function folderTemplate(state = {

  /**
   * "byId" is an object map of all folderTemplate items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ folderTemplate
   * any component that creates a new folderTemplate object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the folderTemplateList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a folderTemplate, the single folderTemplate
   ...
   *
   * For example, when fetching a nested list of folderTemplates by type and color you
   * would write something like:
   * let list = folderTemplateStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT FOLDER TEMPLATE ACTIONS
     */
    case Actions.REQUEST_DEFAULT_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_FOLDER_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_FOLDER_TEMPLATE_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_FOLDER_TEMPLATE_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE FOLDER TEMPLATE ACTIONS
     */
    case Actions.REQUEST_SINGLE_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_FOLDER_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_FOLDER_TEMPLATE_TO_MAP: {
      // add this folderTemplate to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_FOLDER_TEMPLATE: {
      // add this folderTemplate to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_FOLDER_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_FOLDER_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_FOLDER_TEMPLATE: {
      if(action.success) {
    ...
    case Actions.REQUEST_DELETE_ROOT_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_ROOT_FOLDER_TEMPLATE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.INVALIDATE_SELECTED_FOLDER_TEMPLATE: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_FOLDER_TEMPLATE_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: folderTemplateList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
    ...
    case UserActions.RECEIVE_USER_LIST: {
      // add items to "byId" before we forward to individual list reducer
      let newIdMap = { ...state.byId };
      if(action.success && action.folderTemplates) {
        for(const item of action.folderTemplates) {
          newIdMap[item._id] = item;
        }
      }
