...
// import clientNote actions
import * as Actions from './clientNoteActions';
import apiUtils from '../../global/utils/api'

/**
 * clientNoteList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'clientNote' reducer (defined
 * below) when dealing with a LIST action
 */
function clientNoteList(state = {
  /**
   ...
   * then recursively return an _additional_ clientNoteList reducer.
   *
   * Otherwise, return the actual clientNote lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientNoteList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: clientNoteList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_NOTE_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_CLIENT_NOTE_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_CLIENT_NOTE_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_CLIENT_NOTE_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_NOTE_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_NOTE_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_NOTE_PAGINATION: {
        return {
          ...state
...
/**
 * Primary clientNote reducer -
 *
 * This is the single source of truth for all things 'clientNote' related within the
 ...
 */
function clientNote(state = {

  /**
   * "byId" is an object map of all clientNote items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ clientNote
   * any component that creates a new clientNote object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the clientNoteList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a clientNote, the single clientNote
   ...
   *
   * For example, when fetching a nested list of clientNotes by type and color you
   * would write something like:
   * let list = clientNoteStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT CLIENT_NOTE ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_NOTE: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_NOTE_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_NOTE_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_NOTE ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_NOTE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_NOTE_TO_MAP: {
      // add this clientNote to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_CLIENT_NOTE: {
      // add this clientNote to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT_NOTE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_CLIENT_NOTE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_CLIENT_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_NOTE: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT_NOTE: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_NOTE: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_CLIENT_NOTE_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: clientNoteList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
