 ...
 *
 * NOTE: this is never called directly. Only by parent 'staff' reducer (defined
...
function staffList(state = {
   ...
   * then recursively return an _additional_ staffList reducer.
   *
   * Otherwise, return the actual staff lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional staffList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: staffList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_STAFF_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_STAFF_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_STAFF_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_STAFF_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_STAFF_FROM_LIST: {
      ...
      case Actions.SET_STAFF_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_STAFF_PAGINATION: {
        return {
          ...state
...
/**
 * Primary staff reducer -
 *
 * This is the single source of truth for all things 'staff' related within the
...
function staff(state = {
  ...
  /**
   * "loggedInByFirm" maps a single staff object to a firm Id 
   *
   * For example, logging into a firm as a staff user, the action/reducer returns
   * loggedInByFirm[firmId].staff 
   * 
  ...
  /**
   * "byId" is an object map of all staff items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  , byId: {}

  /**
   * "defaultItem" defines fields for a _new_ staff
   * any component that creates a new staff object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the staffList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a staff, the single staff
   ...
   *
   * For example, when fetching a nested list of staffs by type and color you
   * would write something like:
   * let list = staffStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * STAFF LOGGED IN BY FIRM ACTIONS 
     */
    case Actions.REQUEST_STAFF_LOGGED_IN_BY_FIRM: {
      nextState = {
        ...state
        , loggedInByFirm: {
          ...state.loggedInByFirm 
          , [action.firmId]: {
            error: null
            , isFetching: true
          }
    ...
    case Actions.RECEIVE_STAFF_LOGGED_IN_BY_FIRM: {
      if(action.success) {
        nextState = {
          ...state
          , loggedInByFirm: {
    ...
    case Actions.INVALIDATE_STAFF_LOGGED_IN_BY_FIRM: {
      nextState = {
        ...state
        , loggedInByFirm: {
          ...state.loggedInByFirm
     ...
     */
    case Actions.REQUEST_DEFAULT_STAFF: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_STAFF: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_STAFF_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_STAFF_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE STAFF ACTIONS
     */
    case Actions.REQUEST_SINGLE_STAFF: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_STAFF: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_STAFF_TO_MAP: {
      // add this staff to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_STAFF: {
      // add this staff to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_STAFF: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_STAFF: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_STAFF: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_STAFF: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_STAFF: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_STAFF: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_STAFF: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_STAFF_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: staffList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
  ...
  nextState.util.getLoggedInByFirm = firmId => {
    /**
     * utility method to return the staff object set on login 
     */
    return nextState.loggedInByFirm[firmId];
  }
