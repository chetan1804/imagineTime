/**
 * Build the ClientPostReply store
 *
...
import * as Actions from './clientPostReplyActions';
import apiUtils from '../../global/utils/api'

/**
 * clientPostReplyList reducer -
 *
 ...
 */
function clientPostReplyList(state = {
  /**
   ...
   * then recursively return an _additional_ clientPostReplyList reducer.
   *
   * Otherwise, return the actual clientPostReply lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientPostReplyList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: clientPostReplyList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_POST_REPLY_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_CLIENT_POST_REPLY_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_CLIENT_POST_REPLY_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_CLIENT_POST_REPLY_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_POST_REPLY_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_POST_REPLY_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_POST_REPLY_PAGINATION: {
        return {
          ...state
...
/**
 * Primary clientPostReply reducer -
 *
 * This is the single source of truth for all things 'clientPostReply' related within the
 ...
 */
function clientPostReply(state = {

  /**
   * "byId" is an object map of all clientPostReply items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ clientPostReply
   * any component that creates a new clientPostReply object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the clientPostReplyList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a clientPostReply, the single clientPostReply
   ...
   *
   * For example, when fetching a nested list of clientPostReplies by type and color you
   * would write something like:
   * let list = clientPostReplyStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT CLIENT_POST_REPLY ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_POST_REPLY: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_POST_REPLY: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_POST_REPLY_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_POST_REPLY_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_POST_REPLY ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_POST_REPLY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_POST_REPLY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_POST_REPLY_TO_MAP: {
      // add this clientPostReply to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_CLIENT_POST_REPLY: {
      // add this clientPostReply to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT_POST_REPLY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT_POST_REPLY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_CLIENT_POST_REPLY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_CLIENT_POST_REPLY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_POST_REPLY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT_POST_REPLY: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_POST_REPLY: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_CLIENT_POST_REPLY_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: clientPostReplyList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
...
export default clientPostReply;
