...
// import mergeField actions
import * as Actions from './mergeFieldActions';
import apiUtils from '../../global/utils/api'

/**
 * mergeFieldList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'mergeField' reducer (defined
 * below) when dealing with a LIST action
 */
function mergeFieldList(state = {
  /**
   ...
   * then recursively return an _additional_ mergeFieldList reducer.
   *
   * Otherwise, return the actual mergeField lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional mergeFieldList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: mergeFieldList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_MERGE_FIELD_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_MERGE_FIELD_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_MERGE_FIELD_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_MERGE_FIELD_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_MERGE_FIELD_FROM_LIST: {
      ...
      case Actions.SET_MERGE_FIELD_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_MERGE_FIELD_PAGINATION: {
        return {
          ...state
          , pagination: action.pagination
        }
      }
      case Actions.SET_MERGE_FIELD_QUERY: {
        return {
          ...state
          , query: action.query
        }
...
/**
 * Primary mergeField reducer -
 *
 * This is the single source of truth for all things 'mergeField' related within the
 ...
 */
function mergeField(state = {

  /**
   * "byId" is an object map of all mergeField items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ mergeField
   * any component that creates a new mergeField object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the mergeFieldList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a mergeField, the single mergeField
   ...
   *
   * For example, when fetching a nested list of mergeFields by type and color you
   * would write something like:
   * let list = mergeFieldStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT MERGE_FIELD ACTIONS
     */
    case Actions.REQUEST_DEFAULT_MERGE_FIELD: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_MERGE_FIELD: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_MERGE_FIELD_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_MERGE_FIELD_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE MERGE_FIELD ACTIONS
     */
    case Actions.REQUEST_SINGLE_MERGE_FIELD: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_MERGE_FIELD: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_MERGE_FIELD_TO_MAP: {
      // add this mergeField to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_MERGE_FIELD: {
      // add this mergeField to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_MERGE_FIELD: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_MERGE_FIELD: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_MERGE_FIELD: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_MERGE_FIELD: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_MERGE_FIELD: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_MERGE_FIELD: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_MERGE_FIELD: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_MERGE_FIELD_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: mergeFieldList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
