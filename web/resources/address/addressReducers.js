 ...
 *
 * NOTE: this is never called directly. Only by parent 'address' reducer (defined
 * below) when dealing with a LIST action
 */
function addressList(state = {
  /**
   ...
   * then recursively return an _additional_ addressList reducer.
   *
   * Otherwise, return the actual address lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional addressList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: addressList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_ADDRESS_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_ADDRESS_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_ADDRESS_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
      ...
      case Actions.ADD_ADDRESS_TO_LIST:
      ...
      case Actions.REMOVE_ADDRESS_FROM_LIST:
        idArray = [...state.items]
      ...
      case Actions.SET_ADDRESS_PAGINATION: {
        return {
          ...state
...
/**
 * Primary address reducer -
 *
 * This is the single source of truth for all things 'address' related within the
...
function address(state = {
  ...
  /**
   * "byId" is an object map of all address items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a address, the single address
  ...
  /**
   * "lists" corresponds to individual instances of the addressList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_ADDRESS: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_ADDRESS: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUESTADDRESSSCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVEADDRESSSCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_ADDRESS: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_ADDRESS: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_ADDRESS_TO_MAP: {
    ...
    case Actions.REQUEST_CREATE_ADDRESS: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_ADDRESS: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_ADDRESS: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_ADDRESS: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_ADDRESS: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_DELETE_ADDRESS: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_ADDRESS: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_ADDRESS_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: addressList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
    case Actions.RECEIVE_ADDRESS_LIST: {
      ...
      let newIdMap = { ...state.addressList };
      if(action.success) {
        for(const item of action.list) {
