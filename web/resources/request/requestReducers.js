 ...
 *
 * NOTE: this is never called directly. Only by parent 'request' reducer (defined
...
function requestList(state = {
   ...
   * then recursively return an _additional_ requestList reducer.
   *
   * Otherwise, return the actual request lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional requestList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: requestList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_REQUEST_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_REQUEST_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_REQUEST_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case ClientActions.RECEIVE_CLIENT_LIST: {
        if(!action.requests) {
          ...
          let idArray = [];
          for(const item of action.requests) {
            idArray.push(item._id);
          }
          return {
            ...state
      ...
      case Actions.ADD_REQUEST_TO_LIST: {
        let idArray = [];
      ...
      case Actions.ADD_REQUESTS_TO_LIST: {
        let idArray = [];
        if(state && state.items) {
          idArray = [...state.items];
        }
      ...
      case Actions.REMOVE_REQUEST_FROM_LIST: {
      ...
      case Actions.SET_REQUEST_PAGINATION: {
        return {
          ...state
...
/**
 * Primary request reducer -
 *
 * This is the single source of truth for all things 'request' related within the
...
function request(state = {
  ...
  /**
   * "byId" is an object map of all request items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ request
   * any component that creates a new request object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the requestList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a request, the single request
   ...
   *
   * For example, when fetching a nested list of requests by type and color you
   * would write something like:
   * let list = requestStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_REQUEST: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_REQUEST: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_REQUEST_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_REQUEST_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_REQUEST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_REQUEST: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_REQUEST_TO_MAP: {
    ...
    case Actions.SET_SELECTED_REQUEST: {
      // add this request to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_REQUESTS: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_REQUESTS: {
      if(action.success) {
        let newIdMap = {...state.byId}; 
        action.requests.forEach(request => newIdMap[request._id] = request)
        nextState = {
          ...state
          , byId: newIdMap
          , selected: {
    ...
    case Actions.REQUEST_UPDATE_REQUEST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_REQUEST: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_BULK_UPDATE_REQUEST_STATUS: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.REQUEST_BULK_DELETE_REQUEST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_BULK_UPDATE_REQUEST_STATUS: {
    ...
    case Actions.RECEIVE_BULK_DELETE_REQUEST: {
    ...
    case Actions.REQUEST_DELETE_REQUEST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_REQUEST: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_REQUEST: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_REQUEST_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: requestList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
    ...
    case ClientActions.RECEIVE_CLIENT_LIST: {
      // add items to "byId" before we forward to individual list reducer
      let newIdMap = { ...state.byId };
      if(action.success && action.requests) {
        for(const item of action.requests) {
          newIdMap[item._id] = item;
        }
      }
