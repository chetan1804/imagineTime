...
// import taskActivity actions
import * as Actions from './taskActivityActions';
import apiUtils from '../../global/utils/api'

/**
 * taskActivityList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'taskActivity' reducer (defined
 * below) when dealing with a LIST action
 */
function taskActivityList(state = {
  /**
   ...
   * then recursively return an _additional_ taskActivityList reducer.
   *
   * Otherwise, return the actual taskActivity lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional taskActivityList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: taskActivityList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_TASK_ACTIVITY_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_TASK_ACTIVITY_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_TASK_ACTIVITY_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_TASK_ACTIVITY_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_TASK_ACTIVITY_FROM_LIST: {
      ...
      case Actions.SET_TASK_ACTIVITY_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_TASK_ACTIVITY_PAGINATION: {
        return {
          ...state
...
/**
 * Primary taskActivity reducer -
 *
 * This is the single source of truth for all things 'taskActivity' related within the
 ...
 */
function taskActivity(state = {

  /**
   * "byId" is an object map of all taskActivity items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ taskActivity
   * any component that creates a new taskActivity object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the taskActivityList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a taskActivity, the single taskActivity
   ...
   *
   * For example, when fetching a nested list of taskActivitys by type and color you
   * would write something like:
   * let list = taskActivityStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_TASK_ACTIVITY: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_TASK_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_TASK_ACTIVITY_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_TASK_ACTIVITY_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     * SINGLE NOTE ACTIONS
     */
    case Actions.REQUEST_SINGLE_TASK_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_TASK_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_TASK_ACTIVITY_TO_MAP: {
      // add this taskActivity to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_TASK_ACTIVITY: {
      // add this taskActivity to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_TASK_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_TASK_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_TASK_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_TASK_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_TASK_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_TASK_ACTIVITY: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_TASK_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_TASK_ACTIVITY_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: taskActivityList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
