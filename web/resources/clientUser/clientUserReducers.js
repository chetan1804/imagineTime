...
// import clientUser actions
import * as Actions from './clientUserActions';
import apiUtils from '../../global/utils/api'

/**
 * clientUserList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'clientUser' reducer (defined
 * below) when dealing with a LIST action
 */
function clientUserList(state = {
  /**
   ...
   * then recursively return an _additional_ clientUserList reducer.
   *
   * Otherwise, return the actual clientUser lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional clientUserList reducer and trying again.
     */

     console.log("args args", nextAction.listArgs);
    return {
      ...state
      , [nextAction.listArgs[0]]: clientUserList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_USER_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      ...
      case Actions.ADD_CLIENT_USER_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_USER_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_USER_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_USER_QUERY: {
        return {
          ...state
          , query: action.query
        }
      }
      case Actions.SET_CLIENT_USER_PAGINATION: {
        return {
          ...state
...
/**
 * Primary clientUser reducer -
 *
 * This is the single source of truth for all things 'clientUser' related within the
 ...
 */
function clientUser(state = {

  /**
   * "loggedInByClient" maps a single clientUser object to a firm Id 
   *
   * For example, logging into a firm as a clientUser user, the action/reducer returns
   * loggedInByClient[clientId].clientUser 
   * 
  ...
  /**
   * "byId" is an object map of all clientUser items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  , byId: {}

  /**
   * "defaultItem" defines fields for a _new_ clientUser
   * any component that creates a new clientUser object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the clientUserList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a clientUser, the single clientUser
   ...
   *
   * For example, when fetching a nested list of clientUsers by type and color you
   * would write something like:
   * let list = clientUserStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     /**
     * CLIENT_USER LOGGED IN BY CLIENT ACTIONS 
     */
    case Actions.REQUEST_CLIENT_USER_LOGGED_IN_BY_CLIENT: {
      nextState = {
        ...state
        , loggedInByClient: {
          ...state.loggedInByClient 
          , [action.clientId]: {
            error: null
            , isFetching: true
          }
    ...
    case Actions.RECEIVE_CLIENT_USER_LOGGED_IN_BY_CLIENT: {
      if(action.success) {
        nextState = {
          ...state
          , loggedInByClient: {
    ...
    case Actions.INVALIDATE_CLIENT_USER_LOGGED_IN_BY_CLIENT: {
      nextState = {
        ...state
        , loggedInByClient: {
          ...state.loggedInByClient
    ...
    /**
     * DEFAULT CLIENT_USER ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_USER: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_USER: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_USER_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_USER_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_USER ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_USER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_USER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_USER_TO_MAP: {
      // add this clientUser to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_CLIENT_USER: {
      // add this clientUser to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_UPDATE_CLIENT_USER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_CLIENT_USER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_USER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.REQUEST_BULK_UPDATE_CLIENT_USER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_BULK_UPDATE_CLIENT_USER: {
    ...
    case Actions.RECEIVE_DELETE_CLIENT_USER: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_USER: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_CLIENT_USER_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: clientUserList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
