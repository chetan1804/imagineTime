...
// import firm actions
import * as Actions from './firmActions';
 ...
 *
 * NOTE: this is never called directly. Only by parent 'firm' reducer (defined
 * below) when dealing with a LIST action
 */
function firmList(state = {
  /**
   ...
   * then recursively return an _additional_ firmList reducer.
   *
   * Otherwise, return the actual firm lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional firmList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: firmList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_FIRM_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_FIRM_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_FIRM_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.REMOVE_FIRM_FROM_LIST: {
      ...
      case Actions.SET_FIRM_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_FIRM_PAGINATION: {
        return {
          ...state
...
/**
 * Primary firm reducer -
 *
 * This is the single source of truth for all things 'firm' related within the
...
function firm(state = {
  ...
  /**
   * "byId" is an object map of all firm items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ firm
   * any component that creates a new firm object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the firmList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a firm, the single firm
   ...
   *
   * For example, when fetching a nested list of firms by type and color you
   * would write something like:
   * let list = firmStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT FIRM ACTIONS
     */
    case Actions.REQUEST_DEFAULT_FIRM: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_FIRM: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_FIRM_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_FIRM_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     * SINGLE FIRM ACTIONS
     */
    case Actions.REQUEST_SINGLE_FIRM: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_FIRM: {
      if(action.success) {

        if (action.subscription && action.subscription.status === "canceled") {
          if (window.currentUser && window.currentUser._id && !window.currentUser.admin) {
    ...
    case Actions.ADD_SINGLE_FIRM_TO_MAP: {
      // add this firm to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_FIRM: {
      // add this firm to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_FIRM: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_FIRM: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_FIRM: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_FIRM: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_FIRM: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_FIRM: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_FIRM: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    /**
     * Clear firm on logout (important for login page)
     */
    ...
    case Actions.SET_FIRM_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: firmList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
    ...
    case UserActions.RECEIVE_USER_LIST: {
      // add items to "byId" before we forward to individual list reducer
      let newIdMap = { ...state.byId };
      if(action.success && action.firms) {
        for(const item of action.firms) {
          newIdMap[item._id] = item;
        }
      }
      nextState = {
