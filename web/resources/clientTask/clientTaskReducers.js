...
// import task actions
import * as Actions from './clientTaskActions';
...
function taskList(state = {
  ...
  if(nextAction.listArgs.length > 0) {
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_CLIENT_TASK_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_CLIENT_TASK_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_CLIENT_TASK_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case FirmActions.RECEIVE_FIRM_LIST: {
        if(!action.clientTasks) {
          ...
          let idArray = [];
          for(const item of action.clientTasks) {
            idArray.push(item._id);
          }
          return {
            ...state
      ...
      case Actions.ADD_CLIENT_TASK_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_CLIENT_TASK_FROM_LIST: {
      ...
      case Actions.SET_CLIENT_TASK_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_CLIENT_TASK_PAGINATION: {
        return {
          ...state
...
function task(state = {
  ...
  /**
   * "defaultItem" defines fields for a _new_ task
   * any component that creates a new task object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  , formHelpers: {
    hasDueDate: 'no'
    , types: ['file', 'text']
    , clientStatuses:  ['open', 'locked', 'awaitingApproval', 'completed', 'none']
    , firmStatuses: ['draft', 'open', 'locked', 'completed', 'awaitingClient']
    , assureSignTemplates: [
      {
        id: '4ff5e474-35a9-4fd5-925b-aaf601252887'
        , description: 'Single signer IRS e-file signature authorization.'
        , name: 'IRS Form 8879 (single)'
        , signerCount: 1 // the number of signers that this template requires.
      }
      , {
        id: '470ce716-a5d6-4a43-878f-aaf40132a85a'
        , description: 'Dual signer IRS e-file signature authorization'
        , name: 'IRS Form 8879 (married)'
        , signerCount: 2
      }
    ]
  }
   ...
   *
   * For example, when fetching a nested list of clientTasks by type and color you
   * would write something like:
   * let list = clientTaskStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT CLIENT_TASK ACTIONS
     */
    case Actions.REQUEST_DEFAULT_CLIENT_TASK: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_CLIENT_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CLIENT_TASK_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_CLIENT_TASK_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE CLIENT_TASK ACTIONS
     */
    case Actions.REQUEST_SINGLE_CLIENT_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_CLIENT_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_CLIENT_TASK_TO_MAP: {
    ...
    case Actions.SET_SELECTED_CLIENT_TASK: {
      // add this task to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_CLIENT_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_CLIENT_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_PREPARE_CLIENT_TASK_FOR_SIGNATURE:
    case Actions.REQUEST_UPDATE_CLIENT_TASK_STATUS:
    case Actions.REQUEST_UPDATE_CLIENT_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_PREPARE_CLIENT_TASK_FOR_SIGNATURE:
    case Actions.RECEIVE_UPDATE_CLIENT_TASK_STATUS:
    case Actions.RECEIVE_UPDATE_CLIENT_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_CLIENT_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_CLIENT_TASK: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_CLIENT_TASK: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case FirmActions.RECEIVE_FIRM_LIST: {
      // add items to "byId" before we forward to individual list reducer
      let newIdMap = { ...state.byId };
      if(action.success && action.clientTasks) {
        for(const item of action.clientTasks) {
          newIdMap[item._id] = item;
        }
      }
