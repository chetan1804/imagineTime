/**
 * Build the FolderPermission store
 *
...
// import folderPermission actions
import * as Actions from './folderPermissionActions';
import apiUtils from '../../global/utils/api';

/**
 * folderPermission reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'folderPermission' reducer (defined
 * below) when dealing with a LIST action
 */
function folderPermissionList(state = {
  /**
   ...
   * then recursively return an _additional_ folderPermissionList reducer.
   *
   * Otherwise, return the actual folderPermissionList lists' store
   */
    if(nextAction.listArgs.length > 0) {
      /**
       * The action is asking for a nested state, like lists[workout][123ABC].
       * Let's nest it by returning an additional folderPermissionList reducer and trying again.
       */
      return {
        ...state
        , [nextAction.listArgs[0]]: folderPermissionList(state[nextAction.listArgs[0]] || {}, nextAction)
      }
    } else {
      /**
       ...
       */
      switch(action.type) {
        case Actions.INVALIDATE_FOLDER_PERMISSION_LIST: {
          return {
            ...state
            , didInvalidate: true
          }
        }
        case Actions.REQUEST_FOLDER_PERMISSION_LIST: {
          return {
            ...state
            , error: null
            , filter: state.filter || {}
        ...
        case Actions.RECEIVE_FOLDER_PERMISSION_LIST: {
          if(!action.success) {
            return {
              ...state
              , error: action.error
...
/**
 * Primary folderPermission reducer -
 *
 * This is the single source of truth for all things 'folderPermission' related within the
...
function folderPermission(state = {
  
  /**
   * "byId" is an object map of all folderPermission items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
   byId: {}

   /**
    * "defaultItem" defines fields for a _new_ folderPermission
    * any component that creates a new folderPermission object should store a copy of this
    * in its state
    */
   , defaultItem: {
     error: null
   ...
   /**
    * "lists" corresponds to individual instances of the folderPermissionList reducer as
    * defined above.
    *
    ...
    *
    * For example, when changing the name of a folderPermission, the single folderPermission
    ...
    *
    * For example, when fetching a nested list of folderPermissions by type and color you
    * would write something like:
    * let list = folderPermissionStore.util.getList('type', 'apparel', 'color', 'black')
    */
   , util: {
     getKeyArrayFromList: () => {
       return null
  ...
  switch(action.type) {
    /**
     * DEFAULT FOLDER PERMISSION ACTIONS
     */
    case Actions.REQUEST_DEFAULT_FOLDER_PERMISSION: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_FOLDER_PERMISSION: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_CREATE_FOLDER_PERMISSION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_FOLDER_PERMISSION: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_FOLDER_PERMISSION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_FOLDER_PERMISSION: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_FOLDER_PERMISSION: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_FOLDER_PERMISSION: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_FOLDER_PERMISSION: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.INVALIDATE_FOLDER_PERMISSION_LIST:
    case Actions.REQUEST_FOLDER_PERMISSION_LIST:
    case Actions.SET_FOLDER_PERMISSION_FILTER:
    case Actions.ADD_FOLDER_PERMISSION_TO_LIST:
    case Actions.REMOVE_FOLDER_PERMISSION_FROM_LIST:
    case Actions.SET_FOLDER_PERMISSION_QUERY:
    case Actions.SET_FOLDER_PERMISSION_PAGINATION: {
      nextState = {
        ...state
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: folderPermissionList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
