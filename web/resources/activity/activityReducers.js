 ...
 *
 * NOTE: this is never called directly. Only by parent 'activity' reducer (defined
...
function activityList(state = {
   ...
   * then recursively return an _additional_ activityList reducer.
   *
   * Otherwise, return the actual activity lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional activityList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: activityList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_ACTIVITY_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_ACTIVITY_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_ACTIVITY_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_ACTIVITY_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_ACTIVITY_FROM_LIST: {
      ...
      case Actions.SET_ACTIVITY_PAGINATION: {
        return {
          ...state
...
/**
 * Primary activity reducer -
 *
 * This is the single source of truth for all things 'activity' related within the
...
function activity(state = {
  ...
  /**
   * "byId" is an object map of all activity items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ activity
   * any component that creates a new activity object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the activityList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a activity, the single activity
   ...
   *
   * For example, when fetching a nested list of activitys by type and color you
   * would write something like:
   * let list = activityStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_ACTIVITY: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_ACTIVITY_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_ACTIVITY_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_ACTIVITY_TO_MAP: {
      // add this activity to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_ACTIVITY: {
      // add this activity to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_ACTIVITY: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_ACTIVITY: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_ACTIVITY: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_ACTIVITY_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: activityList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
