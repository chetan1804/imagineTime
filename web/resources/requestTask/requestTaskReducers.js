...
// import requestTask actions
import * as Actions from './requestTaskActions';
import apiUtils from '../../global/utils/api'

/**
 * requestTaskList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'requestTask' reducer (defined
 * below) when dealing with a LIST action
 */
function requestTaskList(state = {
  /**
   ...
   * then recursively return an _additional_ requestTaskList reducer.
   *
   * Otherwise, return the actual requestTask lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional requestTaskList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: requestTaskList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_REQUEST_TASK_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_REQUEST_TASK_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_REQUEST_TASK_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_REQUEST_TASK_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_REQUEST_TASK_FROM_LIST: {
      ...
      case Actions.SET_REQUEST_TASK_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_REQUEST_TASK_PAGINATION: {
        return {
          ...state
...
/**
 * Primary requestTask reducer -
 *
 * This is the single source of truth for all things 'requestTask' related within the
...
function requestTask(state = {
  ...
  /**
   * "byId" is an object map of all requestTask items in the store. The map's keys are
  ...
  /**
   * "defaultItem" defines fields for a _new_ requestTask
   * any component that creates a new requestTask object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the requestTaskList reducer as
   * defined above.
   *
  ...
  , displayColumns: null
  , filterNames: {clientFilter: null, statusFilter: null, requestListFilter: null}
  , filterData: {requestListId: -1, clientId: -1}
  , filter: {
    firmId: null
    , orderBy: 'id'
   ...
   *
   * For example, when changing the name of a requestTask, the single requestTask
   ...
   *
   * For example, when fetching a nested list of requestTasks by type and color you
   * would write something like:
   * let list = requestTaskStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.SET_REQUEST_TASK_LINK_NULL: {
      nextState = {
        ...state
        , byHex: {}
        , byId: {}
    ...
    case Actions.REQUEST_DEFAULT_REQUEST_TASK: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_REQUEST_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_REQUEST_TASK_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_REQUEST_TASK_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     * SINGLE NOTE ACTIONS
     */
    case Actions.REQUEST_SINGLE_REQUEST_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_REQUEST_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_REQUEST_TASK_TO_MAP: {
      // add this requestTask to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_REQUEST_TASK: {
      // add this requestTask to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_REQUEST_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_REQUEST_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_REQUEST_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_REQUEST_TASK: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_REQUEST_TASK: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.REQUEST_SINGLE_REQUEST_TASK_LINK_BY_HEX: {
      nextState = {
        ...state
        , selectedHex: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_REQUEST_TASK_LINK_BY_HEX: {
      // console.log(action)
      if(action.success) {
        nextState = {
          ...state
    ...
    case Actions.RECEIVE_DELETE_REQUEST_TASK: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_REQUEST_TASK: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_REQUEST_TASK_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: requestTaskList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
    ...
    case Actions.SET_REQUESTTASK_LIST2_FILTER: {
      nextState = {
        ...state
    ...
    case Actions.SET_REQUESTTASK_LIST2_DISPLAY_COLUMNS: {
      nextState = {
        ...state
