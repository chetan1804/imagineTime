...
// import components
import RequestTaskList from '../components/RequestTaskList.js.jsx';
...
class WorkspaceRequestTasks extends Binder {
  ...
  _handleUpdateRequestTask(action, requestTask) {
    ...
    if ((action === "published" || action === "completed") && requestTask) {

      requestTask["_firm"] = match.params.firmId;
      // requestTask["status"] = action;
      ...
      dispatch(requestTaskActions.sendUpdateRequestTask(requestTask)).then(json => {
        if (!json.success) {
          ...
          dispatch(activityActions.invalidateList(...['_client', match.params.clientId]));
        }
        this.setState({ submitting: false });
      });
    } else if (action === "changes") {
      this.setState({ selectedRequestTask: requestTask, roleModal: "request_task_change" });
    } else {
      this.setState({ selectedRequestTask: requestTask, roleModal: "request_task_list" });
    }
  }
  ...
  render() {
    ...
    console.log("isEmpty", isEmpty, "isFetching", isFetching)
        ...
        { isEmpty ?
          ...
          <div style={{ opacity: isFetching ? 0.5 : 1 }}>
            <RequestTaskList
              ...
              handleRequestTaskShowModal={() => this.setState({ roleModal: "request_task_list" })}
              selectedTaskIds={selectedTaskIds}
              handleTaskBulkEdit={() => this.setState({ requestTaskBulkEdit: true, roleModal: "request_task_list", selectedRequestTask: {} })}
            />
            {
              !isEmpty && !isFetching ?
              <div>
                <ModalComponent 
                  ...
                  selectedTaskIds={selectedTaskIds}
                  requestTaskBulkEdit={requestTaskBulkEdit}
                />
                <TransitionGroup>
                  <CSSTransition
                    key={location.key}
...
WorkspaceRequestTasks.defaultProps = {
