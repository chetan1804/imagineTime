...
import * as Actions from './folderActions';
 ...
 *
 * NOTE: this is never called directly. Only by parent 'folder' reducer (defined
 * below) when dealing with a LIST action
 */
function folderList(state = {
  /**
   ...
   * then recursively return an _additional_ folderList reducer.
   *
   * Otherwise, return the actual folder lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional folderList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: folderList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_FOLDER_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_FOLDER_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_FOLDER_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case ClientActions.RECEIVE_CLIENT_LIST: {
        if(!action.folders) {
          ...
          let idArray = [];
          for(const item of action.folders) {
            idArray.push(item._id);
          }
          return {
            ...state
      ...
      case Actions.ADD_FOLDER_TO_LIST: {
        let idArray = [];
      ...
      case Actions.ADD_FOLDERS_TO_LIST: {
        let idArray = [];
        if(state && state.items) {
          idArray = [...state.items];
        }
      ...
      case Actions.REMOVE_FOLDER_FROM_LIST: {
      ...
      case Actions.SET_FOLDER_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_FOLDER_PAGINATION: {
        return {
          ...state
...
/**
 * Primary folder reducer -
 *
 * This is the single source of truth for all things 'folder' related within the
...
function folder(state = {
  ...
  /**
   * "byId" is an object map of all folder items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ folder
   * any component that creates a new folder object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the folderList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a folder, the single folder
   ...
   *
   * For example, when fetching a nested list of folders by type and color you
   * would write something like:
   * let list = folderStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_FOLDER: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_FOLDER: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_FOLDER_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_FOLDER_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_FOLDER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_FOLDER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_FOLDER_TO_MAP: {
      // add this folder to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_FOLDER: {
      // add this folder to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_FOLDERS: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_FOLDERS: {
      if(action.success) {
        let newIdMap = {...state.byId}; 
        action.folders.forEach(folder => newIdMap[folder._id] = folder)
        nextState = {
          ...state
          , byId: newIdMap
          , selected: {
    ...
    case Actions.REQUEST_UPDATE_FOLDER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_FOLDER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_BULK_UPDATE_FOLDER_STATUS: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.REQUEST_BULK_DELETE_FOLDER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_BULK_UPDATE_FOLDER_STATUS: {
    ...
    case Actions.RECEIVE_BULK_DELETE_FOLDER: {
    ...
    case Actions.REQUEST_DELETE_FOLDER: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_FOLDER: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_FOLDER: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_FOLDER_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: folderList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
    ...
    case ClientActions.RECEIVE_CLIENT_LIST: {
      // add items to "byId" before we forward to individual list reducer
      let newIdMap = { ...state.byId };
      if(action.success && action.folders) {
        for(const item of action.folders) {
          newIdMap[item._id] = item;
        }
      }
