...
// import phoneNumber actions
import * as Actions from './phoneNumberActions';
import apiUtils from '../../global/utils/api'

/**
 * phoneNumberList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'phoneNumber' reducer (defined
 * below) when dealing with a LIST action
 */
function phoneNumberList(state = {
  /**
   ...
   * then recursively return an _additional_ phoneNumberList reducer.
   *
   * Otherwise, return the actual phoneNumber lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional phoneNumberList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: phoneNumberList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_PHONE_NUMBER_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_PHONE_NUMBER_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_PHONE_NUMBER_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
      ...
      case Actions.ADD_PHONE_NUMBER_TO_LIST:
      ...
      case Actions.REMOVE_PHONE_NUMBER_FROM_LIST:
        idArray = [...state.items]
      ...
      case Actions.SET_PHONE_NUMBER_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_PHONE_NUMBER_PAGINATION: {
        return {
          ...state
...
/**
 * Primary phoneNumber reducer -
 *
 * This is the single source of truth for all things 'phoneNumber' related within the
 ...
 */
function phoneNumber(state = {

  /**
   * "byId" is an object map of all phoneNumber items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a phoneNumber, the single phoneNumber
  ...
  /**
   * "lists" corresponds to individual instances of the phoneNumberList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
    /**
     * DEFAULT PHONE_NUMBER ACTIONS
     */
    case Actions.REQUEST_DEFAULT_PHONE_NUMBER: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_PHONE_NUMBER: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUESTPHONE_NUMBERSCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVEPHONE_NUMBERSCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    /**
     * SINGLE PHONE_NUMBER ACTIONS
     */
    case Actions.REQUEST_SINGLE_PHONE_NUMBER: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_PHONE_NUMBER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_PHONE_NUMBER_TO_MAP: {
      // add this phoneNumber to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_PHONE_NUMBER: {
    ...
    case Actions.REQUEST_CREATE_PHONE_NUMBER: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_PHONE_NUMBER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_PHONE_NUMBER: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_PHONE_NUMBER: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_PHONE_NUMBER: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_DELETE_PHONE_NUMBER: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_PHONE_NUMBER: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_PHONE_NUMBER_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: phoneNumberList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
