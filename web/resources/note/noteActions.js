...
export function fetchSingleNoteById(id) {
  return dispatch => {
    dispatch(requestSingleNote(id))
    return apiUtils.callAPI(`/api/notes/${id}`)
...
export const REQUEST_NOTE_SCHEMA = "REQUEST_NOTE_SCHEMA";
function requestNoteSchema(id) {
  return {
    type: REQUEST_NOTE_SCHEMA
  }
}
 export const RECEIVE_NOTE_SCHEMA = "RECEIVE_NOTE_SCHEMA";
...
export function addNoteToList(item, ...listArgs) {
  if(listArgs.length === 0) {
    listArgs = ["all"];
  }
...
/**
 * NoteList2 screen related actions that are dispatched for reducers to
 * store information in application-level (redux) store.
 *
 * For NoteList2 screen, the only things we want to persist in the
 * application store are last filter and its pagination information i.e. rows
 * per page and current pag number.
 */

 export const SET_NOTELIST2_FILTER = "SET_NOTELIST2_FILTER"
 export function setNoteList2Filter(filterNames, filter) {
   return {
     filterNames
     , filter
     , type: SET_NOTELIST2_FILTER
   }
 }

export const SET_NOTELIST2_DISPLAY_COLUMNS = "SET_NOTELIST2_DISPLAY_COLUMNS"
export function setNoteList2Displayolumns(displayColumns) {
  return {
    displayColumns
    , type: SET_NOTELIST2_DISPLAY_COLUMNS
  }
}
