...
class FileSyncData {
  ...
  eSyncState status;
  FileSyncData next;
  String get uri => data.relativePath;
...
class FileSyncList {
  ...
  Callback<FileSync> onAddedActive = Callback();
  ...
  bool get isAddNewOkay => _totalQueue == 0;
  ...
  bool moveWaitingToActive(String uri) {
  ...
  void addSync(FileSync newSync) {
    if (newSync.status.value == eSyncState.Synced)
      Log.write('File ' + newSync.relativePath + ' is already synced.',
          tag: 'SyncController::add', type: eLogType.WARNING);
    activeSync.add(newSync);
    newSync.sync();
    onAddedActive(newSync);
  ...
  bool isContains(String uri) {
    return getActive(uri) != null || getWaiting(uri) != null;
  }

  bool isContainsWaiting(String uri) {
    return getWaiting(uri) != null;
  }

  FileSync getActive(String pUri) {
  ...
  FileSyncData getWaiting(String pUri) {
  ...
  FileSyncData popWaiting(String remove) {
    var current = nextInLine;
  ...
  void updateStatus(eSyncState pStatus, {FileData data}) {
    var waiting = getWaiting(data.relativePath);
    // is currently waiting?
    if (waiting != null) {
      waiting.status = pStatus;
      waiting.data = data;
    } else {
      var active = getActive(data.relativePath);
