...
namespace ImagineTimeMail
{
    public class MapiClient : IEmailClient
    {
        ...
        public bool IsMapiClientInstalled
        {
            get
            {
                var mailClient = registry.GetValue(@"\SOFTWARE\Clients\Mail", "");
                if (mailClient != null)
                {
                    return true;
                }

                mailClient = registry.GetValue(@"\SOFTWARE\Clients\Mail", "");
    ...
    public class MapiMailMessage
    {
        ...
        private string _LogErrorMapi(int errorCode)
        {
            ...
            {
                // Warn("Error sending MAPI Email. Error: " + error + " (code = " + errorCode + ").");
            }
